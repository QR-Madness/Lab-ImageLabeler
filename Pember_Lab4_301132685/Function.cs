...
using Pember_Lab4_301132685.Models;
using Pember_Lab4_301132685.ImageTools;


// Assembly attribute to enable the Lambda function's JSON input to be converted into a .NET class.
[assembly: LambdaSerializer(typeof(Amazon.Lambda.Serialization.SystemTextJson.DefaultLambdaJsonSerializer))]

namespace Pember_Lab4_301132685;
...
public class Function
{
    ...
    public Function(IAmazonS3 s3Client, IAmazonRekognition rekognitionClient, AmazonDynamoDBClient dbClient, float minConfidence)
    {
    ...
    public async Task FunctionHandler(S3Event input, ILambdaContext context)
    {
        foreach (var record in input.Records)
        {
            ...
            // generate the image's thumbnail
            var imgStream = await S3Client.GetObjectStreamAsync(record.S3.Bucket.Name, record.S3.Object.Key, null);
            var imgBytes = new byte[imgStream.Length];
            imgStream.BeginRead(imgBytes, 0, 0, null, null);

            try
            {
                await S3Client.PutObjectAsync(new()
                {
                    Key = "Thumbnails/" + record.S3.Object.Key,
                    InputStream = new MemoryStream(Convert.FromBase64String(Thumbnail.GetConvertedImage(imgBytes)))
                });
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"Couldn't save thumbnail | Details: {ex.Message}");
            }
            // This image data is inserted into the DB for managed records
            S3UploadImageData imageData = new()
            {
                ObjectKey = record.S3.Object.Key,
                // create a new ACL for the object and it's thumbnail
                S3Acl = S3Client.GetPreSignedURL(new()
                {
                    BucketName = record.S3.Bucket.Name,
                    Key = record.S3.Object.Key,
                    Expires = DateTime.Now.AddDays(300)
                }),
                S3AclForThumbnail = S3Client.GetPreSignedURL(new()
                {
                    BucketName = record.S3.Bucket.Name,
                    Key = "Thumbnails/" + record.S3.Object.Key,
                    Expires = DateTime.Now.AddDays(300)
            ...
            Debug.WriteLine($"Looking for labels in image {record.S3.Bucket.Name}:{record.S3.Object.Key}");
            ...
            foreach (var label in detectResponses.Labels)
            {
                if (tags.Count < 10)
                {
                    ...
                    Debug.WriteLine($"\tFound Label {label.Name} with confidence {label.Confidence}");
                    // add the tag to the s3 tags list
                    tags.Add(new Tag { Key = label.Name, Value = label.Confidence.ToString() });
                    // add the tag to the DB (Dynamo) tags list
                    imageData.Tags.Add(new UploadImageLabel { Label = label.Name, Confidence = label.Confidence.ToString(), AdditionalInfo = label.Categories.ToString() });
                }
                else
                {
                    ...
                    Debug.WriteLine($"\tSkipped label {label.Name} with confidence {label.Confidence} because the maximum number of tags has been reached");
                }
            }
            // insert records into DB
            try
            {
                await this.DbClient.PutItemAsync(new()
                {
                    TableName = "Lab4",
                    Item = imageData.ToDocument().ToAttributeMap()
                });
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error inserting item into DynamoDB: {ex.Message}");
                Debug.WriteLine($"Error inserting item into DynamoDB: {ex.Message}");
            }
            // put tags onto the s3 object
